...
void dump_data(uint8_t* data, uint32_t size, uint16_t sum16_val)
{
	int idx;
	int hdr_num = 8;
	int line_num = 16;
	...
	std::cout << std::dec << "["  << __FUNCTION__ << ":"  << __LINE__ << "]" << " DATA : "  << size << std::endl;
	for (idx = 0; idx < hdr_num; ++idx, data++)
	{
		std::cout << std::uppercase << std::setfill('0') << std::setw(2) << std::hex << (int)(*data) << std::nounitbuf;
	}

	for (idx = 0; idx < size - hdr_num; ++idx, data++)
	{
		if (idx % line_num == 0) { 
	...
	std::cout << std::endl;
	std::cout << "SUM16:" << std::uppercase << std::setfill('0') << std::setw(4) << std::hex << sum16_val << std::nounitbuf << std::endl;
	std::cout.flags(oldFlags);
	std::cout << std::dec << "[" << __FUNCTION__ << ":" << __LINE__ << "]" << std::endl;
}
...
uint16_t sum16(uint8_t* data, uint32_t size)
{
	...
	ret = (uint16_t)(sum & 0xffff);
	// dump_data(data, size, ret);
	return ret;
}
